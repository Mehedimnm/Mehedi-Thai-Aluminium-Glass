  ...
  return (
    <div className="relative w-full group" ref={dropdownRef}>
      <div 
        ...
        className={`flex items-center justify-between bg-white border-2 rounded-xl px-4 h-[52px] cursor-pointer transition-all duration-300 ${isOpen ? 'border-indigo-500 ring-4 ring-indigo-500/10' : 'border-gray-100 hover:border-gray-200'}`}
      >
        <div className="flex items-center gap-3 overflow-hidden w-full">
          <div className={`p-1.5 rounded-lg transition-colors ${isOpen ? 'bg-indigo-100 text-indigo-600' : 'bg-gray-50 text-gray-400'}`}>
          ...
          >
            <div className="p-3 border-b border-gray-100 bg-gray-50/50">
              <div className="relative">
                ...
                <input ref={searchInputRef} type="text" placeholder="Type to search..." className="w-full bg-white border border-gray-200 rounded-lg pl-9 pr-3 py-2 text-sm font-medium text-slate-700 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition-all" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/>
              </div>
            </div>
            <div className="max-h-[250px] overflow-y-auto custom-scrollbar p-1">
              {filteredOptions.length > 0 ? (
                filteredOptions.map((opt) => {
                  ...
                  return (
                    <div key={opt._id || opt.value} onClick={() => { onChange(opt._id || opt.value); setIsOpen(false); setSearchTerm(''); }} className={`px-3 py-2.5 rounded-lg cursor-pointer flex justify-between items-center transition-colors ${isSelected ? 'bg-indigo-50 text-indigo-700' : 'hover:bg-gray-50 text-slate-700'}`}>
                      <div>
...
const CreateInvoice = ({ editData }) => {
  ...
  const paymentOptions = [{ _id: 'Cash', name: 'Cash', value: 'Cash' }, { _id: 'Bkash', name: 'Bkash', value: 'Bkash' }, { _id: 'Nagad', name: 'Nagad', value: 'Nagad' }, { _id: 'Bank', name: 'Bank Transfer', value: 'Bank' }];
  ...
  useEffect(() => {
    if (editData && customers.length > 0) {
      setEditingInvoiceId(editData._id); 
      setCurrentInvoiceNo(editData.invoiceNo);
      ...
      setDiscount(pmt.discount || 0);
      setPaid(pmt.paid || 0);
      setPaymentMethod(pmt.method || 'Cash');

      showMessage('success', 'Ready to Update Invoice!');
    }
  }, [editData, customers]);
  ...
  const due = grandTotal - Number(paid);
  ...
  const handleCreateInvoice = async (shouldPrint) => {
    ...
    try {
      ...
      if (res.data.status === 'Success') {
        ...
        if(shouldPrint) { 
            const savedData = { ...invoiceData, invoiceNo: newData.invoiceNo, date: new Date() };
            navigate('/print-invoice', { state: { invoiceData: savedData } });
        } else { 
            setEditingInvoiceId(newData._id);
            setCurrentInvoiceNo(newData.invoiceNo);
            showMessage('success', 'Invoice Saved! You can print now.'); 
        }
      }
    } catch (err) { showMessage('error', 'Error Saving Invoice!'); }
    setLoading(false);
  };

  const handleUpdateInvoice = async () => {
    if (!editingInvoiceId) { showMessage('error', 'No ID Found!'); return; }
    setLoading(true);
    try {
      const customer = customers.find(c => c._id === selectedCustomer);
      const invoiceData = { 
  ...
  const handleDirectPrint = () => {
      ...
      const dataToPrint = {
          ...
          invoiceNo: currentInvoiceNo || 'DRAFT',
          date: new Date(),
          customer: { name: customer.name, mobile: customer.mobile, address: customer.address },
          items: cart,
          payment: { subTotal, discount, grandTotal, paid, due, method: paymentMethod }
      };
      navigate('/print-invoice', { state: { invoiceData: dataToPrint } });
  };
  ...
  return (
    <div className="flex flex-col h-full md:h-[calc(100vh-120px)] relative overflow-y-auto md:overflow-hidden pb-24 md:pb-0 bg-[#f8fafc]">
              ...
              <div className="flex flex-col gap-3 mb-4 pb-4 border-b border-gray-100">
                <div className="flex justify-between items-center">
                    <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                        <List className="w-5 h-5 text-indigo-600"/> Select Items
                    </h3>
                ...
                <div className="relative">
                    ...
                    <input 
                        ...
                        className="w-full bg-gray-50 border border-gray-200 rounded-xl pl-9 pr-3 py-2 text-sm font-medium outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-200"
                    />
                </div>
              </div>
                    ...
                    filteredModalProducts.map((prod) => {
                        ...
                        return (
                            <div 
                                ...
                                className={`p-3 rounded-xl border flex items-center justify-between cursor-pointer transition-all ${isSelected ? 'bg-indigo-50 border-indigo-200 shadow-sm' : 'bg-white border-gray-100 hover:bg-gray-50'}`}
                            >
                                <div>
                                    <p className={`text-sm font-bold ${isSelected ? 'text-indigo-700' : 'text-slate-700'}`}>{prod.name}</p>
        ...
        {deleteIndex !== null && (
          <div className="fixed inset-0 z-[10000] flex items-center justify-center px-4">
            ...
            <motion.div 
                ...
                className="bg-white rounded-[30px] p-8 w-full max-w-sm shadow-2xl relative z-10 text-center border border-white/50"
            >
              <div className="w-20 h-20 bg-red-50 rounded-full flex items-center justify-center mx-auto mb-6 border-4 border-red-100">
                <Trash2 className="w-10 h-10 text-red-500" />
              </div>
              <h3 className="text-2xl font-extrabold text-slate-900 mb-2">Remove Item?</h3>
              <p className="text-gray-500 text-sm font-medium leading-relaxed mb-8">
                Are you sure you want to remove this item from the list?
              </p>
              <div className="flex gap-3">
                <button onClick={() => setDeleteIndex(null)} className="flex-1 py-3.5 rounded-xl border border-gray-200 text-gray-600 font-bold text-sm hover:bg-gray-50 transition">Cancel</button>
                <button onClick={executeDelete} className="flex-1 py-3.5 rounded-xl bg-red-500 text-white font-bold text-sm hover:bg-red-600 shadow-lg shadow-red-500/30 transition">Yes, Remove</button>
              </div>
            </motion.div>
          </div>
        )}
      ...
      {/* --- MAIN INPUT CARD --- */}
      <div className={`bg-white p-5 md:p-6 rounded-[25px] shadow-sm border mb-5 shrink-0 z-20 relative mt-5 transition-all duration-300 ${editingIndex !== null ? 'border-indigo-200 ring-2 ring-indigo-100 shadow-indigo-100' : 'border-gray-200'}`}>
        
        {editingIndex !== null && <div className="absolute -top-3 left-6 bg-indigo-600 text-white text-[10px] font-bold px-3 py-1 rounded-full flex items-center gap-1 shadow-lg shadow-indigo-200 z-30 tracking-wide uppercase"><Pencil className="w-3 h-3"/> Editing Item #{editingIndex + 1}</div>}
        
        {editingInvoiceId && (
            <div className="absolute top-6 right-6">
                <button onClick={resetForm} className="text-xs bg-indigo-50 px-3 py-1.5 rounded-lg font-bold text-indigo-600 hover:bg-indigo-100 border border-indigo-100 transition-all flex items-center gap-1">
        ...
        <div className="flex flex-col gap-5 mt-1">
            ...
            <div className="grid grid-cols-3 gap-4 w-full">
                <div className="space-y-1.5">
                    <div onClick={() => setCalcBy('qty')} className={`h-5 text-[10px] font-bold uppercase cursor-pointer flex items-center gap-1.5 transition-colors ${calcBy === 'qty' ? 'text-indigo-600' : 'text-gray-400 hover:text-gray-600'}`}>
                        {calcBy === 'qty' ? <CheckSquare className="w-3 h-3"/> : <Square className="w-3 h-3"/>} Quantity
                    </div>
                    <input type="number" value={qty} onChange={e => setQty(e.target.value)} className={`w-full h-[52px] border-2 rounded-xl px-2 text-center font-bold text-slate-800 outline-none focus:border-indigo-500 transition-all duration-300 ${calcBy === 'qty' ? 'bg-indigo-50/30 border-indigo-200 text-indigo-900' : 'bg-gray-50/50 border-gray-100'}`}/>
                </div>
                <div className="space-y-1.5">
                    <div onClick={() => setCalcBy('feet')} className={`h-5 text-[10px] font-bold uppercase cursor-pointer flex items-center gap-1.5 transition-colors ${calcBy === 'feet' ? 'text-indigo-600' : 'text-gray-400 hover:text-gray-600'}`}>
                        {calcBy === 'feet' ? <CheckSquare className="w-3 h-3"/> : <Square className="w-3 h-3"/>} Measurement
                    </div>
                    <input type="number" value={feet} onChange={e => setFeet(e.target.value)} placeholder="0" className={`w-full h-[52px] border-2 rounded-xl px-2 text-center font-bold text-slate-800 outline-none focus:border-indigo-500 transition-all duration-300 ${calcBy === 'feet' ? 'bg-indigo-50/30 border-indigo-200 text-indigo-900' : 'bg-gray-50/50 border-gray-100'}`}/>
                </div>
                <div className="space-y-1.5">
                    <label className="h-5 flex items-center text-[10px] font-bold text-gray-400 uppercase gap-1">Rate (Price)</label>
                    <input type="number" value={price} onChange={e => setPrice(e.target.value)} className="w-full h-[52px] border-2 border-gray-100 bg-gray-50/50 rounded-xl px-2 text-center font-bold text-slate-800 outline-none focus:border-indigo-500 focus:bg-indigo-50/30 transition-all duration-300"/>
                </div>
            </div>
                ...
                {selectedProduct && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden relative">
                        <div className="flex justify-between items-end mb-1.5">
                            ...
                            <button onClick={() => setShowDescModal(true)} className="text-[10px] font-bold bg-indigo-50 text-indigo-600 px-2 py-1 rounded-md border border-indigo-100 hover:bg-indigo-100 transition flex items-center gap-1">
                                <List className="w-3 h-3"/> Select from List
                            </button>
                        </div>
                        <textarea value={description} onChange={handleDescriptionChange} onKeyDown={handleDescriptionKeyDown} placeholder="Type description or select from list..." className="w-full p-4 bg-gray-50/50 border-2 border-gray-100 rounded-xl h-24 resize-none outline-none focus:bg-white focus:border-slate-800 transition-all duration-300 text-sm font-medium text-slate-600"/>
                    </motion.div>
                )}
            </AnimatePresence>
            ...
            <div className="flex gap-4 pt-2">
                ...
                <button onClick={handleAddOrUpdate} className={`flex-1 h-[54px] rounded-xl font-bold text-white flex justify-center items-center gap-2 shadow-lg shadow-slate-200 active:scale-[0.98] transition-all duration-300 ${editingIndex !== null ? 'bg-indigo-600 hover:bg-indigo-700 shadow-indigo-200' : 'bg-slate-900 hover:bg-slate-800'}`}>{editingIndex !== null ? <><RefreshCw className="w-5 h-5"/> Update Item</> : <><Plus className="w-5 h-5"/> Add To Invoice</>}</button>
            </div>
        </div>
      </div>
      ...
      <div className="flex-1 bg-white rounded-[25px] shadow-sm border border-gray-200 overflow-hidden flex flex-col mb-4 min-h-[300px] z-10 relative">
        <div className="p-4 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center backdrop-blur-sm">
            <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold uppercase text-slate-700 tracking-wide">Invoice Items</span>
            </div>
                    ...
                    <tbody className="text-sm">
                        {cart.map((item, i) => (
                            <motion.tr 
                                ...
                                key={i} 
                                className={`border-b border-gray-50 last:border-0 group hover:bg-gray-50/50 transition-colors ${editingIndex === i ? 'bg-indigo-50/50' : ''}`}
                            >
      ...
      <div className="bg-white rounded-t-[30px] md:rounded-[25px] shadow-[0_-10px_40px_rgba(0,0,0,0.08)] border border-gray-100 p-6 shrink-0 z-30 relative">
        <div className="flex flex-col md:flex-row gap-8">
           <div className="w-full md:w-1/3 space-y-1.5">
               <label className="h-5 flex items-center text-[10px] font-bold text-gray-400 uppercase ml-1 gap-1"><Wallet className="w-3 h-3"/> Payment Method</label>
               <PremiumDropdown options={paymentOptions} value={paymentMethod} onChange={setPaymentMethod} placeholder="Select Method" icon={CreditCard} type="payment"/>
           </div>
           ...
           <div className="w-full md:w-2/3 flex flex-col gap-4">
               ...
               <div className="flex justify-between items-baseline px-2 border-t border-dashed border-gray-200 pt-3">
                   ...
                   <span className="text-3xl font-black text-indigo-600 tracking-tight">৳ {grandTotal.toLocaleString()}</span>
               </div>

               <div className="flex gap-4">
                   <div className="flex-1 relative group">
                       <span className="absolute left-4 top-3.5 text-[10px] font-bold text-emerald-500 uppercase tracking-wide z-10">PAID</span>
                       <input type="number" value={paid} onChange={e => setPaid(e.target.value)} className="w-full border-2 border-emerald-100 bg-emerald-50/30 rounded-xl pl-4 pr-4 pt-8 pb-2 font-black text-emerald-700 outline-none focus:border-emerald-500 focus:bg-white transition-all text-lg h-[60px]"/>
                   </div>
                   <div className="flex-1 bg-red-50 border-2 border-red-100 rounded-xl p-3 flex flex-col justify-center items-end relative overflow-hidden h-[60px]">
                       <div className="absolute top-0 right-0 p-4 opacity-10"><AlertTriangle className="w-12 h-12 text-red-600"/></div>
                       <span className="text-[10px] font-bold text-red-400 uppercase tracking-wide mb-1 z-10">DUE AMOUNT</span>
                       <span className="font-black text-red-600 text-xl z-10">৳ {due > 0 ? due.toLocaleString() : 0}</span>
                   </div>
               </div>
               
               <div className="flex gap-3 mt-2 border-t border-gray-100 pt-4">
                 {editingInvoiceId ? (
                     <button onClick={handleUpdateInvoice} className="flex-1 py-4 rounded-xl font-bold text-white bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-200 active:scale-[0.98] transition-all flex justify-center items-center gap-2">
                        {loading ? 'Updating...' : <><RefreshCw className="w-5 h-5"/> Update Invoice</>}
                     </button>
                 ) : (
                     <button onClick={() => handleCreateInvoice(false)} className="flex-1 py-4 rounded-xl font-bold text-white bg-slate-900 hover:bg-slate-800 shadow-lg shadow-slate-200 active:scale-[0.98] transition-all flex justify-center items-center gap-2">
                        {loading ? 'Saving...' : <><Save className="w-5 h-5"/> Save Invoice</>}
                     </button>
                 )}
                 <button onClick={() => editingInvoiceId ? handleDirectPrint() : handleCreateInvoice(true)} className="flex-1 py-4 rounded-xl font-bold text-slate-700 bg-gray-100 hover:bg-gray-200 border border-gray-200 shadow-sm active:scale-[0.98] transition-all flex justify-center items-center gap-2">
